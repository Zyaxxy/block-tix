...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <Card className="event-card gradient-border overflow-hidden group hover:scale-105 transition-all duration-300 h-full flex flex-col">
  <Link to={`/events/${event.id}`} className="relative block overflow-hidden aspect-video">
        ...
        />
        {event.featured && (
          <Badge className="absolute top-2 left-2 score-badge backdrop-blur-sm">
            Featured
          </Badge>
        )}
        {isSoldOut && (
          <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
            <Badge variant="destructive" className="score-badge text-lg px-4 py-2">
              Sold Out
            </Badge>
          </div>
        )}
  ...
  <div className="flex items-center text-sm text-muted-foreground space-x-2 font-serif">
      ...
      <CardContent className="pb-2 flex-grow font-serif">
    ...
    <CardFooter className="flex justify-between border-t pt-4">
        <div>
      <p className="text-lg font-semibold text-gradient font-serif">{formatPrice(event.price, event.currency)}</p>
